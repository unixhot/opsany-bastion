<template>
    <div class="online_main">
        <ContentHeader>
            <div slot="docs">
                当前正在使用堡垒机的登录会话，操作强制下线会中断用户的当前请求链接。
            </div>
        </ContentHeader>
        <a-card>
            ...
            <a-table :loading="tableLoading" @change="onChange" :pagination="pagination" :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }" :columns="columns" :data-source="tableData">
                <template slot="action" slot-scope="text,record">
                    <a v-if="$store.state.btnAuth.btnAuth.bastion_session_off_line" @click="offline(record)">强制下线</a>
                </template>
            </a-table>
            <a-button v-if="tableData.length>0" :disabled="selectedRow.length==0" icon="delete" @click="batchDelete" style="float:left;margin:-50px 10px 0 0">批量下线</a-button>
        </a-card>
    </div>
</template>
