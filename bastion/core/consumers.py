...
class WebSSH(WebsocketConsumer):
    ...
    def client_ssh_by_password(self, ip, port, username, password):
        try:
            ...
            self.ssh.connect(hostname=ip, port=port, username=username, password=password, timeout=3)
            return True, ""
    ...
    def client_ssh_by_ssh_key(self, ip, port, ssh_key, passphrase):
        ...
        try:
            ...
            self.ssh.connect(hostname=ip, port=port, pkey=pri_key, timeout=3)
            return True, ""
        except socket.timeout:
            return False, WebSocketStatusCode.TIME_OUT
        except Exception as e:
    ...
    def _create_ssh_link(self, credential, host, password):
        ...
        if credential.login_type == CredentialModel.LOGIN_AUTO:
            if credential.credential_type == CredentialModel.CREDENTIAL_PASSWORD:
                ...
                status, code = self.client_ssh_by_password(host.host_address, host.port, login_name, password)
            else:
                password = credential.passphrase
                ssh_key = credential.ssh_key
                status, code = self.client_ssh_by_ssh_key(host.host_address, host.port, ssh_key, password)
        else:
            if credential.credential_type == CredentialModel.CREDENTIAL_PASSWORD:
                login_name = credential.login_name
                status, code = self.client_ssh_by_password(host.host_address, host.port, login_name, password)
